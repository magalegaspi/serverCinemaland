...
exports.readSucursales = async (req, res) => {
  ...
  try {
    ...
    res.status(200);
    res.send(await sucursalService.getTodasSucursales(paramOrdenar));
  } catch (error) {
    console.log("Error en readSucursales - " + error);
    res
      .status(500)
      .send({ code: 500, message: "Error al obtener las sucursales" });
  }
};

exports.readSucursalOrdered = async (req, res) => {
  console.log(req);
  const sucursal = req.params.sucursal ?? "";
  const sucursalFiltered = await sucursalService.getSucursalesFiltered(
    sucursal
  );
...
exports.createSucursal = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ code: 500, message: "Error al agregar la sucursal" });
  }
};
...
exports.deleteSucursal = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ code: 500, message: "Error al eliminar la sucursal" });
  }
};
...
exports.updateSucursal = async (req, res) => {
  try {
    ...
    const sucursales = await sucursalService.updateSucursal(
      id,
      sucursalActualizada
    );
